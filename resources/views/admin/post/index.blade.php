...
<post-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/posts') }}'"
    :trans="{{ json_encode(trans('brackets/admin-ui::admin.dialogs')) }}" inline-template>
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                        href="{{ url('admin/posts/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{
                        trans('admin.post.actions.create') }}</a>
                </div>
                <div class="card-body" v-cloak>
                    <div class="card-block">
                        <form @submit.prevent="">
                            <div class="row justify-content-md-between">
                                ...
                                <div class="col col-lg-2 col-xl-6 form-group">
                                    <a href="{{ url('admin/posts/sort') }}" class="btn btn-primary">{{ trans("admin.post.actions.sort") }}</a>
                                </div>
                        ...
                        <table class="table table-hover table-listing">
                            <thead>
                                <tr>
                                    ...
                                    <th is='sortable' :column="'category_id'">{{ trans('admin.post.columns.category_id')
                                        }}</th>
                                    </th>
                                    <th is='sortable' :column="'published_at'">{{
                                        trans('admin.post.columns.published_at') }}</th>
                                    ...
                                    <td class="bg-bulk-info d-table-cell text-center" colspan="8">
                                        <span class="align-middle font-weight-light text-dark">{{
                                            ...
                                            clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                @click="onBulkItemsClickedAll('/admin/posts')"
                                        ...
                                        <span class="pull-right pr-2">
                                            <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                @click="bulkDelete('/admin/posts/bulk-destroy')">{{
                        ...
                        <div class="no-items-found" v-if="!collection.length > 0">
                            ...
                            <a class="btn btn-primary btn-spinner" href="{{ url('admin/posts/create') }}"
                                role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.post.actions.create')
                                }}</a>
                        </div>
                    </div>
                </div>
